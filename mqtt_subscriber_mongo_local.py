...
def subscribe(client: mqtt_client, database: str, topic: str):
    ...
    def on_message(client, userdata, msg):
        ...
        print(
            f"Recebeu `{msg.payload.decode()}` do tópico `{msg.topic}` -> coleção `{colecao}`"
        )
...
def send_to_mongo(payload: str, database: str, collection: str) -> None:
    ...
    try:
        ...
        filename = "log_subscriber_mongo-{}.txt".format(
            datetime.now().strftime("%Y-%m-%d-%H:%M:%S")
        )
        with open(filename, "w") as file:
            msg = "JSONDecodeError {} {}".format(
